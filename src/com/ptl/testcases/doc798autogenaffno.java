...
public class doc798autogenaffno extends TestBase {
	@BeforeSuite
	public void init(){
		initConfiguration();
		APPLICATION_LOGS.debug("Configuration file initialized in New Cost Statement Test");
		initDriver();
		APPLICATION_LOGS.debug("Web Driver initialized in New Cost Statement Test");
	}

	@Test(dataProvider="getCreateNewCostStatementData")
	public void CreateNewCostStatementTest(Hashtable<String, String> data) {
		if(!TestUtil.isTestCaseRunmodeYes("CostState New", xls) || data.get("Runmode").equals("No"))
			throw new SkipException("Skipping the Create New Cost Statement test");
		...
		;
		CostStatementAffidavitDetailsPage costStatementPage = homePage.gotoCostStatementAffidavitDetailsPage();
	//	costStatementPage.selectDateFromCalendar(data.get("Valid Through"));
	//	costStatementPage.selectDateFromCalendar(data.get("To"));
		costStatementPage.doCreateNewCostStatement(data.get("Status"), data.get("Document Type"), data.get("StatementNo"),
				data.get("Exporter"), data.get("FTS"), data.get("Product"), data.get("Quantity"), data.get("Unit"), 
